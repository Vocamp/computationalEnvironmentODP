...
def includeA(tag):
    ...
    if h4Depth < h3Depth or before(h4Heading, h3Heading, temp) and h4Heading in levels:
        return h4Depth > levels[h4Heading]

    elif h3Heading in levels:
        return h3Depth > levels[h3Heading];
...
# getting the Linux distributions from a separate page
baseUrl = 'https://en.wikipedia.org/wiki/List_of_Linux_distributions'

doc = get(baseUrl).text
soup = BeautifulSoup(doc, 'html.parser')

ignore = {'v', 'Linux kernel', 'Distributions', 'Adoption', 'Category', 'Linux'}

items = soup.select('th')
...
for i in items:
    ...
    # only keep the left-most column of tables
    prevSib = i.find_previous_sibling('td')
    if prevSib and i.parent == prevSib.parent: continue

    prevSib = i.find_previous_sibling('th')
    if prevSib and i.parent == prevSib.parent: continue
